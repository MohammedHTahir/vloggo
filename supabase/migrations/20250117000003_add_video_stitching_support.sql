...
SET status = 'failed'
WHERE status NOT IN ('processing', 'completed', 'failed', 'stitching');

-- Update status CHECK constraint to include 'stitching'
ALTER TABLE public.video_generations
DROP CONSTRAINT IF EXISTS video_generations_status_check;

ALTER TABLE public.video_generations
ADD CONSTRAINT video_generations_status_check
CHECK (status IN ('processing', 'completed', 'failed', 'stitching'));
